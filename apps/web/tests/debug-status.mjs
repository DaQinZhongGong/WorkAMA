...
page.on('response', (response) => {
  if (response.url().includes('/public/platform-info')) {
    console.log('platform-info response:', response.status(), response.url())
  }
})
...
await page.screenshot({ path: '/tmp/status-debug.png', fullPage: true })
await browser.close()
